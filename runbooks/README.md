# dynamic-bonding-curve Runbooks
...
### Start a Surfnet and run the deployment runbook

```console
$ NO_DNA=1 surfpool start --no-tui --no-studio --ci
```
...
```console
$ NO_DNA=1 surfpool start --watch --no-tui --no-studio
```

### Execute an existing runbook
```console
$ NO_DNA=1 surfpool run deployment --env localnet --explain --unsupervised
```
