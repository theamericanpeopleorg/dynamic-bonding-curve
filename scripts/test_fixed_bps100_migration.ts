...
function usage() {
  console.log(`Usage:
  pnpm exec tsx scripts/test_fixed_bps100_migration.ts [options]
...
function startSurfpool(mainnetRpcUrl: string): SurfpoolProcess {
  ...
  const child = spawn("surfpool", ["start", "--rpc-url", mainnetRpcUrl], {
    cwd: process.cwd(),
    env: {
      ...process.env,
      NO_DNA: "1",
...
async function fetchPoolState(rpcUrl: string, pool: PublicKey) {
  ...
  const state = await (program.account as any).virtualPool.fetch(pool);
  return {
    pool: pool.toBase58(),
