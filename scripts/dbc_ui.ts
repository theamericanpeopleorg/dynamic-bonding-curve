...
import { buy, createConfig, createPool, poolInfo } from "./dbc_tool/commands";
import {
  DEFAULT_RPC_URL,
  MAINNET_USDC_MINT,
  buildClient,
...
function usage() {
  console.log(`Usage:
  pnpm exec tsx scripts/dbc_ui.ts [--host <HOST>] [--port <PORT>] [--rpc-url <URL>]
...
async function route(
  ...
  if (method === "POST" && requestUrl.pathname === "/api/create-pool") {
    ...
    const result = await createPool(config, { rpcUrl: options.rpcUrl });
    sendJson(res, publicKeyResultToBase58(result));
    return;
  }
...
async function readPoolDashboard(pool: string, rpcUrl: string) {
  ...
  <script>
    ...
    async function refreshNow() {
      ...
      try {
        ...
        if (state.health.ok && pool) {
          state.dashboard = await getJson("/api/pool-info?pool=" + encodeURIComponent(pool));
    ...
    async function createConfigAction() {
      await withButton(el.createConfigButton, async () => {
        ...
        const result = await postJson("/api/create-config", { quoteMint });
    ...
    async function createPoolAction() {
      await withButton(el.createPoolButton, async () => {
        ...
        const result = await postJson("/api/create-pool", { config });
